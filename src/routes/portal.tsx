import { readableQuoteTerms } from "../../supabase/functions/_shared/quote-service-scope"
...
import { OfficialQuoteDetails, type OfficialQuote } from "@/components/official-quote-details"
...
type Estimate = OfficialQuote & { id: string; estimate_number: string; title: string | null; status: string; valid_until: string | null; total: number; created_at: string; estimated_duration_minutes: number | null; crew_size: number }
type Invoice = { id: string; invoice_number: string; title: string | null; status: string; issue_date: string; due_date: string | null; subtotal: number; discount_total: number; tax_rate: number; tax_total: number; total: number; amount_paid: number; balance_due: number; currency: string; job_id: string | null; terms: string | null }
...
function CustomerPortal() {
  ...
  async function loadPortal() {
    ...
    const [customerResult, estimatesResult, invoicesResult, jobsResult, signaturesResult, bookingsResult] = await Promise.all([
      ...
      supabase.from("estimates").select("id,estimate_number,title,status,valid_until,total,subtotal,discount_total,tax_rate,tax_total,currency,notes,terms,created_at,estimated_duration_minutes,crew_size").eq("customer_id", customerId).order("created_at", { ascending: false }),
      supabase.from("invoices").select("id,invoice_number,title,status,issue_date,due_date,subtotal,discount_total,tax_rate,tax_total,total,amount_paid,balance_due,currency,job_id,terms").eq("customer_id", customerId).order("issue_date", { ascending: false }),
  ...
  function durationText(estimate: Estimate) {
    ...
    if (!minutes) return language === "fr" ? "Durée à confirmer" : "Duration to be confirmed"
    const hours = minutes / 60
    const crew = estimate.crew_size || 1
    const number = new Intl.NumberFormat(language === "fr" ? "fr-CA" : "en-CA", { maximumFractionDigits: 1 })
    return language === "fr"
      ? `${number.format(hours)} h sur place · équipe de ${crew} · ${number.format(hours * crew)} heures-personnes`
      : `${number.format(hours)} on-site hours · crew of ${crew} · ${number.format(hours * crew)} labour-hours`
  }
  function quoteDate(value: string) {
    return new Intl.DateTimeFormat(language === "fr" ? "fr-CA" : "en-CA", { dateStyle: "medium", timeZone: "America/Toronto" }).format(new Date(value.length === 10 ? `${value}T12:00:00` : value))
  }
...
function invoiceDate(value: string, language: "en" | "fr") {
  return new Intl.DateTimeFormat(language === "fr" ? "fr-CA" : "en-CA", { year: "numeric", month: "short", day: "numeric" }).format(new Date(value.length === 10 ? `${value}T12:00:00` : value))
}
...
function InvoiceCard({ invoice, payments, onView, onDownload }: { invoice: Invoice; payments: Payment[]; onView: () => void; onDownload: () => void }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="text-lg font-bold text-slate-950">{invoice.invoice_number}</p><Badge status={invoice.status} /></div><p data-i18n-ignore="true" className="mt-1 text-sm text-slate-500">{language === "fr" ? "Émise le" : "Issued"} {invoiceDate(invoice.issue_date, language)}{invoice.due_date ? ` · ${language === "fr" ? "Échéance" : "Due"} ${invoiceDate(invoice.due_date, language)}` : ""}</p>{lastPayment && <p className="mt-2 text-sm text-slate-600"><span data-i18n-ignore="true">{language === "fr" ? "Dernier paiement :" : "Last payment:"}</span> {paymentLabel[lastPayment.method] ?? lastPayment.method} · {formatCad(lastPayment.amount)}</p>}</div><div className="sm:text-right"><p className="text-sm text-slate-500">Total</p><p className="text-2xl font-bold text-slate-950">{formatCad(invoice.total)}</p><p className={`mt-1 text-sm font-semibold ${paid ? "text-emerald-700" : "text-amber-700"}`}>{paid ? "Paid in full" : `${language === "fr" ? "Solde" : "Balance"} ${formatCad(invoice.balance_due)}`}</p></div></div><div className="flex flex-wrap gap-2 border-t bg-slate-50 px-5 py-4"><button type="button" onClick={onView} className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-100"><Eye className="mr-2 h-4 w-4" />View invoice</button><button type="button" onClick={onDownload} className="inline-flex min-h-11 items-center justify-center rounded-lg bg-teal-800 px-4 py-2 text-sm font-semibold text-white hover:bg-teal-900"><Download className="mr-2 h-4 w-4" />Download PDF</button></div></article>
}
...
function InvoiceDialog({ invoice, customer, items, payments, photos, onClose, onDownload }: { invoice: Invoice; customer: Customer; items: InvoiceItem[]; payments: Payment[]; photos: InvoicePhoto[]; onClose: () => void; onDownload: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 p-4"><div className="mx-auto my-4 w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex flex-wrap items-center justify-between gap-4 border-b bg-slate-950 px-4 py-4 text-white sm:px-6"><div><p className="text-xs font-semibold tracking-[0.2em] text-teal-600">OTTAWA MULTISERVICES</p><p className="mt-1 text-lg font-bold">Ottawa Multiservices Group Inc.</p></div><div className="flex gap-2"><button onClick={onDownload} className="inline-flex min-h-11 items-center justify-center rounded-lg bg-teal-800 px-3 py-2 text-sm font-semibold"><Download className="mr-2 h-4 w-4" />PDF</button><button onClick={onClose} aria-label={language === "fr" ? "Fermer la facture" : "Close invoice"} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg border border-slate-700 p-2"><X className="h-5 w-5" /></button></div></div><div className="p-4 sm:p-8"><div className="flex flex-col gap-6 border-b pb-6 sm:flex-row sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Billed to</p><p className="mt-2 text-lg font-bold">{customer.first_name} {customer.last_name ?? ""}</p><p className="text-sm text-slate-600">{address || "Address on file"}</p><p className="break-all text-sm text-slate-600">{customer.email}</p>{customer.phone && <p className="text-sm text-slate-600">{customer.phone}</p>}</div><div className="sm:text-right"><p className="text-3xl font-black text-slate-950">INVOICE</p><p className="mt-2 font-bold">{invoice.invoice_number}</p><p className={`mt-2 inline-block rounded-full px-3 py-1 text-xs font-bold ${paid ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{paid ? "PAID" : "AMOUNT DUE"}</p><p data-i18n-ignore="true" className="mt-3 text-sm text-slate-500">{language === "fr" ? "Émise le" : "Issued"} {invoiceDate(invoice.issue_date, language)}</p>{invoice.due_date && <p data-i18n-ignore="true" className="text-sm text-slate-500">{language === "fr" ? "Échéance" : "Due"} {invoiceDate(invoice.due_date, language)}</p>}</div></div><div className="mt-6 overflow-x-auto rounded-xl border"><table className="w-full min-w-[560px] text-sm"><thead className="bg-slate-50 text-slate-600"><tr><th className="px-4 py-3 text-left">Description</th><th className="px-4 py-3 text-center">Qty</th><th className="px-4 py-3 text-right">Unit price</th><th className="px-4 py-3 text-right">Amount</th></tr></thead><tbody className="divide-y">{items.length ? items.map((item) => <tr key={item.id}><td className="px-4 py-3 font-medium">{item.description}</td><td className="px-4 py-3 text-center">{Number(item.quantity)}</td><td className="px-4 py-3 text-right">{formatCad(item.unit_price)}</td><td className="px-4 py-3 text-right font-semibold">{formatCad(item.line_total)}</td></tr>) : <tr><td colSpan={4} className="px-4 py-5 text-center text-slate-500">Service details are not available.</td></tr>}</tbody></table></div>{photos.length > 0 && <section className="mt-6"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-slate-950">Completed work photos</h3><p className="text-sm text-slate-500">Photos attached to this invoice after the job.</p></div><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">{photos.length} photo(s)</span></div><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">{photos.map((photo) => <a key={photo.id} href={photo.url} target="_blank" rel="noreferrer" className="overflow-hidden rounded-xl border bg-slate-50"><img src={photo.url} alt={photo.caption ?? "Work completed"} className="h-40 w-full object-cover"/><div className="p-2 text-xs font-medium text-slate-600">{photo.caption || "After work"}</div></a>)}</div></section>}<div className="mt-6 grid gap-6 md:grid-cols-[minmax(0,1fr)_320px]"><div className="min-w-0">{payments.length > 0 && <div><h3 className="font-bold">Payments</h3><div className="mt-3 overflow-x-auto rounded-xl border"><table className="w-full min-w-[500px] text-sm"><thead className="bg-slate-50"><tr><th className="px-3 py-2 text-left">Date</th><th className="px-3 py-2 text-left">Method</th><th className="px-3 py-2 text-left">Reference</th><th className="px-3 py-2 text-right">Amount</th></tr></thead><tbody className="divide-y">{payments.map((payment) => <tr key={payment.id}><td data-i18n-ignore="true" className="px-3 py-2">{invoiceDate(payment.payment_date, language)}</td><td className="px-3 py-2">{paymentLabel[payment.method] ?? payment.method}</td><td className="max-w-[180px] break-all px-3 py-2">{payment.reference || "—"}</td><td className="px-3 py-2 text-right font-semibold">{formatCad(payment.amount)}</td></tr>)}</tbody></table></div></div>}</div><div className="rounded-xl border bg-slate-50 p-5 text-sm"><div className="flex justify-between py-1"><span>Subtotal before tax</span><strong>{formatCad(invoice.subtotal)}</strong></div>{Number(invoice.discount_total || 0) > 0 && <div className="flex justify-between py-1"><span>Discount</span><strong>-{formatCad(invoice.discount_total)}</strong></div>}<div className="flex justify-between py-1"><span>{quoteTaxLabel(Number(invoice.tax_rate))}</span><strong>{formatCad(invoice.tax_total)}</strong></div><div className="mt-2 flex justify-between border-t pt-3 text-lg"><span>Total</span><strong>{formatCad(invoice.total)}</strong></div><div className="flex justify-between py-1 text-emerald-700"><span>Paid</span><strong>{formatCad(invoice.amount_paid)}</strong></div><div className="mt-2 flex justify-between border-t pt-3 text-lg"><span>Balance</span><strong>{formatCad(invoice.balance_due)}</strong></div></div></div>{invoice.terms && <section className="mt-6 rounded-xl border bg-slate-50 p-4 text-sm"><h3 className="font-bold">Terms / Conditions</h3><p className="mt-2 whitespace-pre-line text-slate-600">{readableQuoteTerms(invoice.terms)}</p></section>}<div className="mt-8 border-t pt-5 text-xs text-slate-500"><p>Ottawa Multiservices Group Inc. · Ottawa, Ontario, Canada · <a href="tel:+16134076699" className="font-semibold text-slate-700">(613) 407-6699</a></p><p>Keep this invoice for your records.</p></div></div></div></div>
}
...
async function downloadInvoicePdf(invoice: Invoice, customer: Customer | null, items: InvoiceItem[], payments: Payment[], photos: InvoicePhoto[] = [], language: "en" | "fr" = "en") {
  ...
  if (invoice.terms) {
    // Separate page keeps the agreed terms readable even on a long invoice.
    doc.addPage(); y = 50;
    doc.setTextColor(15,23,42); doc.setFont("helvetica", "bold"); doc.setFontSize(13);
    doc.text(tPdf("Invoice terms", "Conditions de facturation"), margin, y); y += 24;
    doc.setFont("helvetica", "normal"); doc.setFontSize(10);
    for (const line of doc.splitTextToSize(readableQuoteTerms(invoice.terms), right - margin)) {
...
function BookingDialog({ estimate, onClose, onConfirm, confirmLabel }: { estimate: Estimate; onClose: () => void; onConfirm: (slot: Slot) => Promise<void> | void; confirmLabel: string }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/70 p-4"><div className="max-h-[calc(100dvh-2rem)] w-full max-w-2xl overflow-y-auto overscroll-contain rounded-2xl bg-white p-4 shadow-2xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-blue-700">APPOINTMENT SELECTION</p><h2 className="mt-1 text-2xl font-bold">Choose a new available time</h2><p data-i18n-ignore="true" className="mt-1 text-sm text-slate-600">{estimate.estimate_number} · {language === "fr" ? "durée estimée" : "estimated duration"} {new Intl.NumberFormat(language === "fr" ? "fr-CA" : "en-CA").format(Number(estimate.estimated_duration_minutes || 0) / 60)} h</p></div><button onClick={onClose} className="min-h-11 shrink-0 rounded-lg border px-3 py-2 text-sm">Close</button></div>{error && <Alert text={error} />}<label className="mt-6 block text-sm font-semibold">Date<input type="date" min={new Date().toISOString().slice(0,10)} value={date} onChange={(e)=>setDate(e.target.value)} className="mt-2 w-full rounded-lg border px-3 py-2"/></label><div className="mt-5"><p className="text-sm font-semibold">Available times</p>{loading ? <p className="mt-3 text-sm text-slate-500">Loading…</p> : slots.length === 0 ? <p className="mt-3 rounded-lg bg-slate-50 p-4 text-sm text-slate-600">No time slots are available for this date. Choose another day.</p> : <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">{slots.map((slot)=><button key={slot.startsAt} type="button" onClick={()=>setSelected(slot)} className={`rounded-lg border px-3 py-3 text-sm font-semibold ${selected?.startsAt===slot.startsAt?"border-blue-700 bg-blue-50 text-blue-800":"hover:border-blue-400"}`}>{slot.label}</button>)}</div>}</div><div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>Important:</strong> Choose the appointment that works for you first. If the quote has not yet been accepted, the time slot will be confirmed when you sign electronically.</div><button type="button" onClick={()=>void confirm()} disabled={!selected || saving} className="mt-5 w-full rounded-lg bg-teal-800 px-4 py-3 font-semibold text-white disabled:opacity-50">{saving?"Confirming…":confirmLabel}</button></div></div>
}

function PortalHeader({ customer, onLogout }: { customer?: Customer | null; onLogout?: () => void }) {
  return <header className="border-b border-teal-100 bg-white/95 text-slate-800 shadow-sm backdrop-blur-xl">
    <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-x-4 gap-y-3 px-4 py-4 sm:px-5">
      <Link to="/" className="min-w-0 text-sm font-bold tracking-wide sm:text-base"><span className="text-teal-700">OTTAWA</span> MULTISERVICES</Link>
      <div className="flex w-full min-w-0 flex-wrap items-center gap-3 sm:w-auto sm:flex-1 sm:justify-end">
        <LanguageSwitcher compact />
        {customer && <span title={customer.email} className="hidden min-w-0 max-w-[18rem] items-center text-sm text-slate-600 md:inline-flex"><UserRound className="mr-1 h-4 w-4 shrink-0" /><span className="truncate">{customer.email}</span></span>}
        {onLogout && <button onClick={onLogout} className="inline-flex min-h-11 shrink-0 items-center rounded-lg border border-teal-200 bg-teal-50 px-3 py-2 text-sm text-teal-800 hover:bg-teal-100"><LogOut className="mr-1 h-4 w-4 shrink-0" />Sign out</button>}
      </div>
    </div>
  </header>
}
