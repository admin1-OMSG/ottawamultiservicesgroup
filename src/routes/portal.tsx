...
function CustomerPortal() {
  ...
  async function loadPortal() {
    ...
    if (accountError || !account) {
      setCustomer(null)
      setError("Aucun dossier client n’est associé à cette adresse. Communiquez avec Ottawa Multiservices Group au (613) 407-6699 pour faire vérifier votre courriel.")
      return
    }
...
async function downloadInvoicePdf(invoice: Invoice, customer: Customer | null, items: InvoiceItem[], payments: Payment[], photos: InvoicePhoto[] = []) {
  ...
  doc.setDrawColor(226,232,240); doc.line(margin,720,right,720); doc.setFont("helvetica","normal"); doc.setFontSize(8); doc.setTextColor(100,116,139); doc.text("Ottawa Multiservices Group Inc. · Ottawa, Ontario, Canada · (613) 407-6699",margin,740); doc.text("Merci pour votre confiance.",right,740,{align:"right"})
  if (photos.length) {
    for (let index = 0; index < photos.length; index++) {
      try {
        ...
        if (index % 2 === 0) { doc.addPage(); doc.setFont("helvetica", "bold"); doc.setFontSize(16); doc.setTextColor(15,23,42); doc.text("Photos du travail terminé", margin, 42) }
      ...
      } catch (photoError) { console.warn("Impossible d'ajouter une photo au PDF", photoError) }
    }
  }
