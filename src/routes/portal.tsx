...
type Invoice = { id: string; invoice_number: string; title: string | null; status: string; issue_date: string; due_date: string | null; subtotal: number; discount_total: number; tax_rate: number; tax_total: number; total: number; amount_paid: number; balance_due: number; currency: string }
...
function CustomerPortal() {
  ...
  async function loadPortal() {
    ...
    const [customerResult, estimatesResult, invoicesResult, jobsResult, signaturesResult, bookingsResult] = await Promise.all([
      ...
      supabase.from("invoices").select("id,invoice_number,title,status,issue_date,due_date,subtotal,discount_total,tax_rate,tax_total,total,amount_paid,balance_due,currency").eq("customer_id", customerId).order("issue_date", { ascending: false }),
...
function InvoiceDialog({ invoice, customer, items, payments, onClose, onDownload }: { invoice: Invoice; customer: Customer; items: InvoiceItem[]; payments: Payment[]; onClose: () => void; onDownload: () => void }) {
  const paid = Number(invoice.balance_due || 0) <= 0 || invoice.status === "paid"
