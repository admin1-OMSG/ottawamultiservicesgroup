...
function CustomerPortal() {
  ...
  const [invoicePhotos, setInvoicePhotos] = useState<Record<string, {id:string;url:string}[]>>({})
  ...
  async function loadPortal() {
    ...
    const [customerResult, estimatesResult, invoicesResult, jobsResult, signaturesResult, bookingsResult] = await Promise.all([
      supabase.from("customers").select("id,first_name,last_name,email,phone,address_line,city,province,postal_code").eq("id", customerId).single(),
    ...
    if (invoiceRows.length) {
      ...
      const jobIds = invoiceRows.map((i)=>i.job_id).filter(Boolean) as string[]
      const photoMap: Record<string,{id:string;url:string}[]> = {}
      if (jobIds.length) {
        const { data: photos } = await supabase.from("job_photos").select("id,job_id,storage_path").in("job_id", jobIds).eq("kind","after")
        for (const photo of photos ?? []) { const { data } = await supabase.storage.from("service-photos").createSignedUrl(photo.storage_path, 3600); if (!data?.signedUrl) continue; const invoice = invoiceRows.find((i)=>i.job_id===photo.job_id); if (invoice) (photoMap[invoice.id] ??= []).push({id:photo.id,url:data.signedUrl}) }
      }
  ...
  function durationText(estimate: Estimate) {
    ...
    return `${hoursText} h sur place · équipe de ${estimate.crew_size || 1} · ${(hours * (estimate.crew_size || 1)).toFixed(1)} h-personnes`
  }

  async function logout() {
    await supabase.auth.signOut()
    setCustomer(null); setEstimates([]); setInvoices([]); setInvoiceItems({}); setInvoicePayments({}); setViewingInvoice(null); setJobs([]); setSignatures({}); setBookings({}); setLoggedIn(false)
  }

  if (!sessionReady) return <div className="grid min-h-screen place-items-center bg-slate-50">Chargement…</div>
  if (!loggedIn) return <div className="min-h-screen bg-slate-50"><PortalHeader /><main className="mx-auto max-w-md px-6 py-20"><div className="rounded-2xl border bg-white p-7 shadow-sm"><p className="text-sm font-semibold text-emerald-700">PORTAIL CLIENT · VERSION 0.8</p><h1 className="mt-2 text-3xl font-bold text-slate-950">Accédez à votre dossier</h1><p className="mt-3 text-slate-600">Recevez un lien sécurisé par courriel. Aucun mot de passe n’est nécessaire.</p>{error && <Alert text={error} />}{message && <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">{message}</div>}<form onSubmit={sendLink} className="mt-6"><label className="text-sm font-semibold">Adresse courriel<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3" placeholder="nom@exemple.ca" /></label><button disabled={sending} className="mt-4 w-full rounded-xl bg-emerald-600 px-4 py-3 font-semibold text-white hover:bg-emerald-700">{sending ? "Envoi…" : "Recevoir mon lien de connexion"}</button></form><p className="mt-5 text-center text-xs text-slate-500">Utilisez la même adresse courriel que celle fournie lors de votre demande de devis.</p></div></main></div>

  return <div className="min-h-screen bg-slate-100"><PortalHeader customer={customer} onLogout={logout} /><main className="mx-auto max-w-6xl space-y-8 px-5 py-8">{error && <Alert text={error} />}<section><p className="text-sm font-semibold text-emerald-700">ESPACE CLIENT</p><h1 className="mt-1 text-3xl font-bold">Bonjour {customer?.first_name ?? ""}</h1><p className="text-slate-600">Consultez et signez vos devis, puis suivez vos factures et interventions.</p></section><div className="grid gap-4 sm:grid-cols-3"><Stat icon={ReceiptText} label="Devis" value={estimates.length} /><Stat icon={FileText} label="Solde à payer" value={formatCad(invoices.reduce((total, invoice) => total + Number(invoice.balance_due), 0))} /><Stat icon={CalendarDays} label="Interventions" value={jobs.length} /></div><Section title="Mes devis"><div className="grid gap-4">{estimates.length === 0 ? <Empty /> : estimates.map((estimate) => { const signature = signatures[estimate.id]; const booking = bookings[estimate.id]; const canRespond = ["sent", "viewed", "pending_review"].includes(estimate.status); return <article id={`estimate-${estimate.id}`} key={estimate.id} className="rounded-xl border bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold text-blue-700">{estimate.estimate_number}</p><h3 className="text-lg font-semibold">{estimate.title || "Devis de service"}</h3><p className="text-sm text-slate-500">Créé le {formatDate(estimate.created_at)}{estimate.valid_until && ` · Valide jusqu’au ${formatDate(estimate.valid_until)}`}</p></div><div className="text-right"><p className="text-xl font-bold">{formatCad(estimate.total)}</p><Badge status={estimate.status} /></div></div><div className="mt-4 rounded-lg bg-slate-50 p-3 text-sm"><strong>Durée estimée :</strong> {durationText(estimate)}</div>{signature && <div className="mt-4 flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0" /><div><p className="font-semibold">Signé électroniquement par {signature.signer_name}</p><p>Le {formatDate(signature.signed_at)}</p></div></div>}{booking && <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><p className="font-semibold">Rendez-vous réservé</p><p className="mt-1">{formatDate(booking.starts_at)} → {formatDate(booking.ends_at)}</p></div>}{canRespond && !signature && <div className="mt-4 flex flex-wrap gap-3 border-t pt-4"><button onClick={() => setBookingEstimate(estimate)} className="inline-flex items-center rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white"><CalendarDays className="mr-2 h-4 w-4" />Choisir un créneau puis signer</button><button onClick={() => void rejectEstimate(estimate.id)} className="rounded-lg border px-4 py-2 text-sm font-semibold">Refuser</button></div>}{estimate.status === "accepted" && signature && !booking && <div className="mt-4 border-t pt-4"><button onClick={() => setBookingEstimate(estimate)} className="inline-flex items-center rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white"><CalendarDays className="mr-2 h-4 w-4" />Choisir mon rendez-vous</button></div>}</article> })}</div></Section><Section title="Mes factures"><div className="grid gap-4">{invoices.length === 0 ? <Empty /> : invoices.map((invoice) => <InvoiceCard key={invoice.id} invoice={invoice} payments={invoicePayments[invoice.id] ?? []} onView={() => setViewingInvoice(invoice)} onDownload={() => void downloadInvoicePdf(invoice, customer, invoiceItems[invoice.id] ?? [], invoicePayments[invoice.id] ?? [])} />)}</div></Section><Section title="Mes interventions"><div className="grid gap-4 md:grid-cols-2">{jobs.length === 0 ? <Empty /> : jobs.map((job) => <article key={job.id} className="rounded-xl border bg-white p-5"><div className="flex justify-between gap-3"><div><p className="text-sm font-semibold text-blue-700">{job.job_number}</p><h3 className="font-bold">{job.title}</h3></div><Badge status={job.status} /></div><p className="mt-3 text-sm text-slate-600">{job.scheduled_start ? formatDate(job.scheduled_start) : "Date à confirmer"}</p>{(job.address_line || job.city) && <p className="mt-1 text-sm text-slate-500">{[job.address_line, job.city].filter(Boolean).join(", ")}</p>}</article>)}</div></Section></main>{viewingInvoice && customer && <InvoiceDialog invoice={viewingInvoice} customer={customer} items={invoiceItems[viewingInvoice.id] ?? []} payments={invoicePayments[viewingInvoice.id] ?? []} photos={invoicePhotos[viewingInvoice.id] ?? []} onClose={() => setViewingInvoice(null)} onDownload={() => void downloadInvoicePdf(viewingInvoice, customer, invoiceItems[viewingInvoice.id] ?? [], invoicePayments[viewingInvoice.id] ?? [])} />}{signingEstimate && signingSlot && customer && <SignatureDialog estimate={signingEstimate} slot={signingSlot} defaultName={`${customer.first_name} ${customer.last_name ?? ""}`.trim()} onClose={() => { setSigningEstimate(null); setSigningSlot(null) }} onSigned={async () => { setSigningEstimate(null); setSigningSlot(null); await loadPortal() }} />}{bookingEstimate && <BookingDialog estimate={bookingEstimate} onClose={() => setBookingEstimate(null)} confirmLabel={signatures[bookingEstimate.id] ? "Confirmer ce rendez-vous" : "Continuer vers la signature"} onConfirm={async (slot) => { const estimate = bookingEstimate; if (!estimate) return; if (signatures[estimate.id]) { const { data: bookingId, error: rpcError } = await supabase.rpc("customer_book_estimate_slot", { p_estimate_id: estimate.id, p_starts_at: slot.startsAt }); if (rpcError) throw rpcError; const notification = await sendCrmEmail({ type: "booking_confirmed", bookingId: String(bookingId) }); if (!notification.ok) console.warn("Booking saved, but confirmation email failed"); setBookingEstimate(null); await loadPortal(); return } setBookingEstimate(null); setSigningSlot(slot); setSigningEstimate(estimate) }} />}</div>
}
...
function InvoiceCard({ invoice, payments, onView, onDownload }: { invoice: Invoice; payments: Payment[]; onView: () => void; onDownload: () => void }) {
  ...
  return <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="text-lg font-bold text-slate-950">{invoice.invoice_number}</p><Badge status={invoice.status} /></div><p className="mt-1 text-sm text-slate-500">Émise le {formatDate(invoice.issue_date)}{invoice.due_date ? ` · Échéance ${formatDate(invoice.due_date)}` : ""}</p>{lastPayment && <p className="mt-2 text-sm text-slate-600">Dernier paiement : {paymentLabel[lastPayment.method] ?? lastPayment.method} · {formatCad(lastPayment.amount)}</p>}</div><div className="sm:text-right"><p className="text-sm text-slate-500">Total</p><p className="text-2xl font-bold text-slate-950">{formatCad(invoice.total)}</p><p className={`mt-1 text-sm font-semibold ${paid ? "text-emerald-700" : "text-amber-700"}`}>{paid ? "Payée en totalité" : `Solde ${formatCad(invoice.balance_due)}`}</p></div></div><div className="flex flex-wrap gap-2 border-t bg-slate-50 px-5 py-4"><button type="button" onClick={onView} className="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-100"><Eye className="mr-2 h-4 w-4" />Voir la facture</button><button type="button" onClick={onDownload} className="inline-flex items-center rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800"><Download className="mr-2 h-4 w-4" />Télécharger PDF</button></div></article>
}

function InvoiceDialog({ invoice, customer, items, payments, photos, onClose, onDownload }: { invoice: Invoice; customer: Customer; items: InvoiceItem[]; payments: Payment[]; photos: {id:string;url:string}[]; onClose: () => void; onDownload: () => void }) {
  const paid = Number(invoice.balance_due || 0) <= 0 || invoice.status === "paid"
  const address = [customer.address_line, customer.city, customer.province, customer.postal_code].filter(Boolean).join(", ")
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 p-4"><div className="mx-auto my-4 w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b bg-slate-950 px-6 py-4 text-white"><div><p className="text-xs font-semibold tracking-[0.2em] text-emerald-400">OTTAWA MULTISERVICES</p><p className="mt-1 text-lg font-bold">Ottawa Multiservices Group Inc.</p></div><div className="flex gap-2"><button onClick={onDownload} className="inline-flex items-center rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold"><Download className="mr-2 h-4 w-4" />PDF</button><button onClick={onClose} className="rounded-lg border border-slate-700 p-2"><X className="h-5 w-5" /></button></div></div><div className="p-6 sm:p-8"><div className="flex flex-col gap-6 border-b pb-6 sm:flex-row sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Facturé à</p><p className="mt-2 text-lg font-bold">{customer.first_name} {customer.last_name ?? ""}</p><p className="text-sm text-slate-600">{address || "Adresse au dossier"}</p><p className="text-sm text-slate-600">{customer.email}</p>{customer.phone && <p className="text-sm text-slate-600">{customer.phone}</p>}</div><div className="sm:text-right"><p className="text-3xl font-black text-slate-950">FACTURE</p><p className="mt-2 font-bold">{invoice.invoice_number}</p><p className={`mt-2 inline-block rounded-full px-3 py-1 text-xs font-bold ${paid ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{paid ? "ACQUITTÉE" : "À PAYER"}</p><p className="mt-3 text-sm text-slate-500">Émise le {formatDate(invoice.issue_date)}</p>{invoice.due_date && <p className="text-sm text-slate-500">Échéance {formatDate(invoice.due_date)}</p>}</div></div><div className="mt-6 overflow-x-auto rounded-xl border"><table className="w-full min-w-[560px] text-sm"><thead className="bg-slate-50 text-slate-600"><tr><th className="px-4 py-3 text-left">Description</th><th className="px-4 py-3 text-center">Qté</th><th className="px-4 py-3 text-right">Prix unitaire</th><th className="px-4 py-3 text-right">Montant</th></tr></thead><tbody className="divide-y">{items.length ? items.map((item) => <tr key={item.id}><td className="px-4 py-3 font-medium">{item.description}</td><td className="px-4 py-3 text-center">{Number(item.quantity)}</td><td className="px-4 py-3 text-right">{formatCad(item.unit_price)}</td><td className="px-4 py-3 text-right font-semibold">{formatCad(item.line_total)}</td></tr>) : <tr><td colSpan={4} className="px-4 py-5 text-center text-slate-500">Détail des prestations non disponible.</td></tr>}</tbody></table></div><div className="mt-6 grid gap-6 md:grid-cols-[1fr_320px]"><div>{payments.length > 0 && <div><h3 className="font-bold">Paiements</h3><div className="mt-3 overflow-hidden rounded-xl border"><table className="w-full text-sm"><thead className="bg-slate-50"><tr><th className="px-3 py-2 text-left">Date</th><th className="px-3 py-2 text-left">Méthode</th><th className="px-3 py-2 text-left">Référence</th><th className="px-3 py-2 text-right">Montant</th></tr></thead><tbody className="divide-y">{payments.map((payment) => <tr key={payment.id}><td className="px-3 py-2">{formatDate(payment.payment_date)}</td><td className="px-3 py-2">{paymentLabel[payment.method] ?? payment.method}</td><td className="px-3 py-2">{payment.reference || "—"}</td><td className="px-3 py-2 text-right font-semibold">{formatCad(payment.amount)}</td></tr>)}</tbody></table></div></div>}</div><div className="rounded-xl border bg-slate-50 p-5 text-sm"><div className="flex justify-between py-1"><span>Sous-total avant taxes</span><strong>{formatCad(invoice.subtotal)}</strong></div>{Number(invoice.discount_total || 0) > 0 && <div className="flex justify-between py-1"><span>Remise</span><strong>-{formatCad(invoice.discount_total)}</strong></div>}<div className="flex justify-between py-1"><span>HST ({formatTaxRate(invoice.tax_rate)} %)</span><strong>{formatCad(invoice.tax_total)}</strong></div><div className="mt-2 flex justify-between border-t pt-3 text-lg"><span>Total</span><strong>{formatCad(invoice.total)}</strong></div><div className="flex justify-between py-1 text-emerald-700"><span>Payé</span><strong>{formatCad(invoice.amount_paid)}</strong></div><div className="mt-2 flex justify-between border-t pt-3 text-lg"><span>Solde</span><strong>{formatCad(invoice.balance_due)}</strong></div></div></div>{photos.length > 0 && <div className="mt-7"><h3 className="font-bold">Photos du travail terminé</h3><p className="mt-1 text-sm text-slate-500">Photos fournies avec votre facture.</p><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">{photos.map((photo)=><a key={photo.id} href={photo.url} target="_blank" rel="noreferrer"><img src={photo.url} alt="Travail terminé" className="h-40 w-full rounded-lg border object-cover"/></a>)}</div></div>}<div className="mt-8 border-t pt-5 text-xs text-slate-500"><p>Ottawa Multiservices Group Inc. · Ottawa, Ontario, Canada</p><p>Conservez cette facture pour vos dossiers.</p></div></div></div></div>
}

async function downloadInvoicePdf(invoice: Invoice, customer: Customer | null, items: InvoiceItem[], payments: Payment[]) {
  if (!customer) return
  ...
  doc.setFont("helvetica", "bold"); doc.setFontSize(24); doc.text("FACTURE", right, 52, { align: "right" }); doc.setFontSize(12); doc.text(invoice.invoice_number, right, 74, { align: "right" })
  doc.setFontSize(9); doc.text(paid ? "ACQUITTEE" : "A PAYER", right, 96, { align: "right" })
  doc.setTextColor(15,23,42); y=150
  doc.setFontSize(9); doc.setTextColor(100,116,139); doc.text("FACTURE A", margin, y); doc.setTextColor(15,23,42); doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.text(`${customer.first_name} ${customer.last_name ?? ""}`.trim(), margin, y+20)
  ...
  doc.setTextColor(100,116,139); doc.text("Date d'emission", right-160, y); doc.setTextColor(15,23,42); doc.text(formatDate(invoice.issue_date), right, y, { align:"right" }); if(invoice.due_date){doc.setTextColor(100,116,139); doc.text("Date d'echeance", right-160, y+18); doc.setTextColor(15,23,42); doc.text(formatDate(invoice.due_date), right, y+18, {align:"right"})}
  y=230; doc.setFillColor(248,250,252); doc.rect(margin,y,right-margin,26,"F"); doc.setFont("helvetica","bold"); doc.setTextColor(51,65,85); doc.text("Description", margin+8,y+17); doc.text("Qte", 360,y+17,{align:"center"}); doc.text("Prix", 455,y+17,{align:"right"}); doc.text("Montant", right-8,y+17,{align:"right"}); y+=38
  ...
  if(!items.length){doc.setTextColor(100,116,139);doc.text("Detail des prestations non disponible.",margin+8,y);y+=24}
  y+=10; const totalsX=360; doc.setFillColor(248,250,252); doc.roundedRect(totalsX,y,right-totalsX,126,7,7,"F"); const totalLine=(label:string,value:string,offset:number,bold=false)=>{doc.setFont("helvetica",bold?"bold":"normal");doc.setTextColor(71,85,105);doc.text(label,totalsX+12,y+offset);doc.setTextColor(15,23,42);doc.text(value,right-12,y+offset,{align:"right"})}; totalLine("Sous-total avant taxes",formatCad(invoice.subtotal),20); let off=38; if(Number(invoice.discount_total||0)>0){totalLine("Remise",`-${formatCad(invoice.discount_total)}`,off);off+=18} totalLine(`HST (${formatTaxRate(invoice.tax_rate)} %)`,formatCad(invoice.tax_total),off);off+=22; totalLine("Total",formatCad(invoice.total),off,true);off+=20; totalLine("Montant paye",formatCad(invoice.amount_paid),off);off+=20; totalLine("Solde",formatCad(invoice.balance_due),off,true); y+=150
  if(payments.length){doc.setFont("helvetica","bold");doc.setFontSize(11);doc.text("Paiements",margin,y);y+=18;doc.setFontSize(9);for(const payment of payments){doc.setFont("helvetica","normal");doc.text(`${formatDate(payment.payment_date)}  ·  ${paymentLabel[payment.method] ?? payment.method}${payment.reference?`  ·  Ref. ${payment.reference}`:""}`,margin,y);doc.setFont("helvetica","bold");doc.text(formatCad(payment.amount),right,y,{align:"right"});y+=18}}
  doc.setDrawColor(226,232,240); doc.line(margin,720,right,720); doc.setFont("helvetica","normal"); doc.setFontSize(8); doc.setTextColor(100,116,139); doc.text("Ottawa Multiservices Group Inc. · Ottawa, Ontario, Canada",margin,740); doc.text("Merci pour votre confiance.",right,740,{align:"right"})
...
function BookingDialog({ estimate, onClose, onConfirm, confirmLabel }: { estimate: Estimate; onClose: () => void; onConfirm: (slot: Slot) => Promise<void> | void; confirmLabel: string }) {
  ...
  async function loadSlots() {
    ...
    setLoading(false)
    if (invokeError || data?.error) { setSlots([]); setError(data?.error || invokeError?.message || "Impossible de charger les disponibilités."); return }
  ...
  async function confirm() {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : "Impossible de confirmer ce créneau.") }
    finally { setSaving(false) }
  }
  return <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/70 p-4"><div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-blue-700">ÉTAPE 1 · CHOIX DU RENDEZ-VOUS</p><h2 className="mt-1 text-2xl font-bold">Choisissez votre créneau avant de signer</h2><p className="mt-1 text-sm text-slate-600">{estimate.estimate_number} · durée estimée {Number(estimate.estimated_duration_minutes || 0) / 60} h</p></div><button onClick={onClose} className="rounded-lg border px-3 py-2 text-sm">Fermer</button></div>{error && <Alert text={error} />}<label className="mt-6 block text-sm font-semibold">Date<input type="date" min={new Date().toISOString().slice(0,10)} value={date} onChange={(e)=>setDate(e.target.value)} className="mt-2 w-full rounded-lg border px-3 py-2"/></label><div className="mt-5"><p className="text-sm font-semibold">Créneaux disponibles</p>{loading ? <p className="mt-3 text-sm text-slate-500">Chargement…</p> : slots.length === 0 ? <p className="mt-3 rounded-lg bg-slate-50 p-4 text-sm text-slate-600">Aucun créneau disponible pour cette date. Choisissez une autre journée.</p> : <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">{slots.map((slot)=><button key={slot.startsAt} type="button" onClick={()=>setSelected(slot)} className={`rounded-lg border px-3 py-3 text-sm font-semibold ${selected?.startsAt===slot.startsAt?"border-blue-700 bg-blue-50 text-blue-800":"hover:border-blue-400"}`}>{slot.label}</button>)}</div>}</div><div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>Important :</strong> choisissez d’abord le rendez-vous qui vous convient. Pour un devis non encore accepté, le créneau sera confirmé définitivement au moment de la signature électronique.</div><button type="button" onClick={()=>void confirm()} disabled={!selected || saving} className="mt-5 w-full rounded-lg bg-blue-700 px-4 py-3 font-semibold text-white disabled:opacity-50">{saving?"Confirmation…":confirmLabel}</button></div></div>
}
