...
async function downloadInvoicePdf(invoice: Invoice, customer: Customer | null, items: InvoiceItem[], payments: Payment[], photos: InvoicePhoto[] = []) {
  ...
  doc.setFont("helvetica", "bold"); doc.setFontSize(24); doc.text("INVOICE", right, 52, { align: "right" }); doc.setFontSize(12); doc.text(invoice.invoice_number, right, 74, { align: "right" })
  doc.setFontSize(9); doc.text(paid ? "PAID" : "AMOUNT DUE", right, 96, { align: "right" })
  doc.setTextColor(15,23,42); y=150
  doc.setFontSize(9); doc.setTextColor(100,116,139); doc.text("BILL TO", margin, y); doc.setTextColor(15,23,42); doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.text(`${customer.first_name} ${customer.last_name ?? ""}`.trim(), margin, y+20)
  ...
  doc.setTextColor(100,116,139); doc.text("Issue date", right-160, y); doc.setTextColor(15,23,42); doc.text(formatDate(invoice.issue_date), right, y, { align:"right" }); if(invoice.due_date){doc.setTextColor(100,116,139); doc.text("Due date", right-160, y+18); doc.setTextColor(15,23,42); doc.text(formatDate(invoice.due_date), right, y+18, {align:"right"})}
  y=230; doc.setFillColor(248,250,252); doc.rect(margin,y,right-margin,26,"F"); doc.setFont("helvetica","bold"); doc.setTextColor(51,65,85); doc.text("Description", margin+8,y+17); doc.text("Qty", 360,y+17,{align:"center"}); doc.text("Price", 455,y+17,{align:"right"}); doc.text("Amount", right-8,y+17,{align:"right"}); y+=38
  ...
  if(!items.length){doc.setTextColor(100,116,139);doc.text("Service details are not available.",margin+8,y);y+=24}
  y+=10; const totalsX=360; doc.setFillColor(248,250,252); doc.roundedRect(totalsX,y,right-totalsX,126,7,7,"F"); const totalLine=(label:string,value:string,offset:number,bold=false)=>{doc.setFont("helvetica",bold?"bold":"normal");doc.setTextColor(71,85,105);doc.text(label,totalsX+12,y+offset);doc.setTextColor(15,23,42);doc.text(value,right-12,y+offset,{align:"right"})}; totalLine("Subtotal before tax",formatCad(invoice.subtotal),20); let off=38; if(Number(invoice.discount_total||0)>0){totalLine("Discount",`-${formatCad(invoice.discount_total)}`,off);off+=18} totalLine(`HST (${formatTaxRate(invoice.tax_rate)} %)`,formatCad(invoice.tax_total),off);off+=22; totalLine("Total",formatCad(invoice.total),off,true);off+=20; totalLine("Amount paid",formatCad(invoice.amount_paid),off);off+=20; totalLine("Balance",formatCad(invoice.balance_due),off,true); y+=150
  if(payments.length){doc.setFont("helvetica","bold");doc.setFontSize(11);doc.text("Payments",margin,y);y+=18;doc.setFontSize(9);for(const payment of payments){doc.setFont("helvetica","normal");doc.text(`${formatDate(payment.payment_date)}  ·  ${paymentLabel[payment.method] ?? payment.method}${payment.reference?`  ·  Ref. ${payment.reference}`:""}`,margin,y);doc.setFont("helvetica","bold");doc.text(formatCad(payment.amount),right,y,{align:"right"});y+=18}}
  doc.setDrawColor(226,232,240); doc.line(margin,720,right,720); doc.setFont("helvetica","normal"); doc.setFontSize(8); doc.setTextColor(100,116,139); doc.text("Ottawa Multiservices Group Inc. · Ottawa, Ontario, Canada · (613) 407-6699",margin,740); doc.text("Thank you for your business.",right,740,{align:"right"})
  if (photos.length) {
    for (let index = 0; index < photos.length; index++) {
      try {
        ...
        if (index % 2 === 0) { doc.addPage(); doc.setFont("helvetica", "bold"); doc.setFontSize(16); doc.setTextColor(15,23,42); doc.text("Completed work photos", margin, 42) }
