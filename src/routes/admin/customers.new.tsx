...
function NewCustomerPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Impossible de créer le client.") } finally { setSaving(false) }
  }

  return <div className="mx-auto max-w-3xl space-y-6"><header><Link to="/admin/customers" className="text-sm font-semibold text-blue-700 hover:underline">← Retour aux clients</Link><h1 className="mt-2 text-3xl font-bold text-slate-950">Nouveau client</h1></header>{error && <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">{error}</div>}<form onSubmit={submit} className="rounded-xl border bg-white p-6 shadow-sm"><div className="grid gap-5 sm:grid-cols-2"><Field label="Prénom *" value={form.first_name} onChange={(v) => setForm({ ...form, first_name: v })} required /><Field label="Nom" value={form.last_name} onChange={(v) => setForm({ ...form, last_name: v })} /><Field label="Courriel *" type="email" value={form.email} onChange={(v) => setForm({ ...form, email: v })} required /><Field label="Téléphone" type="tel" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} /><div className="sm:col-span-2"><Field label="Adresse" value={form.address_line} onChange={(v) => setForm({ ...form, address_line: v })} /></div><Field label="Ville" value={form.city} onChange={(v) => setForm({ ...form, city: v })} /><Field label="Province" value={form.province} onChange={(v) => setForm({ ...form, province: v })} /><Field label="Code postal" value={form.postal_code} onChange={(v) => setForm({ ...form, postal_code: v })} /><div className="sm:col-span-2"><label className="text-sm font-medium text-slate-700">Notes internes</label><textarea value={form.internal_notes} onChange={(e) => setForm({ ...form, internal_notes: e.target.value })} rows={4} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" /></div></div><div className="mt-6 flex justify-end gap-3"><Link to="/admin/customers" className="rounded-lg border px-4 py-2 font-medium">Annuler</Link><button disabled={saving} className="rounded-lg bg-emerald-600 px-4 py-2 font-semibold text-white disabled:opacity-60">{saving ? "Création…" : "Créer le client"}</button></div></form></div>
}
