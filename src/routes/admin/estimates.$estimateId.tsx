...
import { termsWithoutScope } from "../../../supabase/functions/_shared/quote-service-scope"
...
function EstimateDetailPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setContractMonths(typed.contract_months == null ? null : Number(typed.contract_months))
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
      <div className="space-y-6">
        ...
        {estimate.service_request_id && <Link to="/admin/estimates/new" search={{ serviceRequestId: estimate.service_request_id }} className="inline-block text-sm font-semibold text-teal-800 underline">Préparer un nouveau devis depuis cette demande / Prepare a new quote from this request</Link>}
        {termsWithoutScope(estimate.terms) && <section className="rounded-xl border bg-white p-5 shadow-sm"><h2 className="text-lg font-bold">Terms</h2><p className="mt-3 whitespace-pre-wrap text-sm text-slate-600">{termsWithoutScope(estimate.terms)}</p></section>}
