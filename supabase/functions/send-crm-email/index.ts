import { renderServiceScopeHtml, readableQuoteTerms } from "../_shared/quote-service-scope.ts"
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.type === "quote_requested") {
      ...
      const serviceScope = renderServiceScopeHtml(estimate.terms, recipientLanguage)
      ...
      html = layout("Your quote is ready", `<p>Hello ${escapeHtml(customer.first_name)},</p><p>We prepared your quote <strong>${escapeHtml(estimate.estimate_number)}</strong> ${tr("for", "pour")} ${escapeHtml(estimate.title || tr("the requested service", "le service demandé"))}.</p><div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:16px;margin:18px 0"><p style="margin:0 0 8px">${tr("Amount for the visit covered by this quote", "Montant de la visite couverte par ce devis")} : <strong>${escapeHtml(displayAmount)}</strong></p><p style="margin:0">Estimated duration: <strong>${escapeHtml(durationText)}</strong></p></div>${billingSummary}<p>${serviceScope ? tr('Your service annex is included below. In the portal, open “View services included in each visit” or the service link beside each visit amount. Calculations and conditions are available separately.', 'Votre annexe des prestations figure ci-dessous. Dans le portail, ouvrez « Voir les prestations comprises à chaque visite » ou le lien près du montant de chaque visite. Les calculs et conditions sont accessibles séparément.') : tr('Review the recorded service lines, notes and terms in the portal. If the scope is incomplete, request a revised quote before signing.', 'Consultez les lignes, notes et conditions enregistrées dans le portail. Si le périmètre est incomplet, demandez un devis révisé avant de signer.')}</p><p>The button below signs you in directly and securely to your quote. You can review it, choose an available appointment, and sign it.</p><p><a href="${escapeHtml(portalLink)}" style="display:inline-block;background:#059669;color:white;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:700">${tr("View my quote, services and calculations", "Voir mon devis, les prestations et les calculs")}</a></p>${serviceScope}`)
      ...
      const invoiceBody = `
        ...
        ${invoice.terms ? `<div style="margin:18px 0;padding:16px;background:#f8fafc;border-radius:8px;font-size:13px;line-height:1.6"><strong>Terms / Conditions</strong><p>${escapeHtml(readableQuoteTerms(invoice.terms)).replace(/\n/g, "<br>")}</p></div>` : ""}
        ${invoicePhotoHtml}
