...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.type === "quote_requested") {
      ...
      let portalLink = `${siteUrl}/portal?estimate=${estimate.id}`
      let { data: linkData, error: linkError } = await admin.auth.admin.generateLink({ type: "magiclink", email: customer.email, options: { redirectTo: portalLink } })
      if (linkError || !linkData?.properties?.action_link) {
        const signup = await admin.auth.admin.generateLink({ type: "signup", email: customer.email, password: crypto.randomUUID(), options: { redirectTo: portalLink } })
        linkData = signup.data
        linkError = signup.error
      }
      if (!linkError && linkData?.properties?.action_link) portalLink = linkData.properties.action_link
      html = layout("Votre devis est prêt", `<p>Bonjour ${escapeHtml(customer.first_name)},</p><p>Nous avons préparé votre devis <strong>${escapeHtml(estimate.estimate_number)}</strong> pour ${escapeHtml(estimate.title || "le service demandé")}.</p><p>Montant : <strong>${money(estimate.total)}</strong></p><p>Cliquez sur le bouton ci-dessous : ce lien sécurisé vous connecte et ouvre directement votre devis, sans demander un deuxième courriel.</p><p><a href="${portalLink}" style="display:inline-block;background:#059669;color:white;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:700">Voir et signer mon devis</a></p>`)
