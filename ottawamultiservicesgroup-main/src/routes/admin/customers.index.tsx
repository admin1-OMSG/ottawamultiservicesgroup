...
function CustomersPage() {
  ...
  return <div className="space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-sm font-semibold text-emerald-700">CRM</p><h1 className="text-3xl font-bold text-slate-950">Clients</h1><p className="mt-1 text-slate-600">Coordonnées et historique commercial de vos clients.</p></div>
      <Link to="/admin/customers/new" className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 font-semibold text-white hover:bg-emerald-700"><UserPlus className="h-4 w-4" /> Nouveau client</Link>
    </header>

    <section className="grid gap-4 sm:grid-cols-3">
      <Metric label="Clients" value={customers.length} />
      <Metric label="Actifs" value={customers.filter((c) => c.status !== "archived").length} />
    ...
    <section className="rounded-xl border bg-white p-4 shadow-sm">
      <label htmlFor="customer-search" className="text-sm font-medium text-slate-700">Rechercher un client</label>
      <div className="relative mt-2"><Search className="absolute left-3 top-2.5 h-5 w-5 text-slate-400" /><input id="customer-search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Nom, courriel, téléphone ou ville" className="w-full rounded-lg border border-slate-300 py-2 pl-10 pr-3 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100" /></div>
    </section>

    {error && <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">{error}</div>}
    <section className="overflow-hidden rounded-xl border bg-white shadow-sm">
      {loading ? <p className="p-8 text-slate-600">Chargement des clients…</p> : filtered.length === 0 ? <p className="p-8 text-center text-slate-500">Aucun client trouvé.</p> : <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead className="bg-slate-100 text-sm text-slate-700"><tr><th className="px-4 py-3">Client</th><th className="px-4 py-3">Téléphone</th><th className="px-4 py-3">Ville</th><th className="px-4 py-3">Depuis</th><th className="px-4 py-3">Action</th></tr></thead><tbody className="divide-y">{filtered.map((customer) => <tr key={customer.id} className="hover:bg-slate-50"><td className="px-4 py-4"><p className="font-semibold text-slate-900">{customer.first_name} {customer.last_name ?? ""}</p><p className="text-sm text-slate-500">{customer.email}</p></td><td className="px-4 py-4">{customer.phone ?? "—"}</td><td className="px-4 py-4">{customer.city ?? "—"}</td><td className="px-4 py-4 text-sm text-slate-600">{formatDate(customer.created_at)}</td><td className="px-4 py-4"><Link to="/admin/customers/$customerId" params={{ customerId: customer.id }} className="font-semibold text-blue-700 hover:underline">Ouvrir</Link></td></tr>)}</tbody></table></div>}
    </section>
  </div>
}
