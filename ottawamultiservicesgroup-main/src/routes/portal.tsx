...
type Estimate = { id: string; estimate_number: string; title: string | null; status: string; valid_until: string | null; total: number; estimated_duration_hours: number | null; crew_size: number; created_at: string }
...
function CustomerPortal() {
  ...
  async function loadPortal() {
    ...
    const [customerResult, estimatesResult, invoicesResult, jobsResult, signaturesResult] = await Promise.all([
      ...
      supabase.from("estimates").select("id,estimate_number,title,status,valid_until,total,estimated_duration_hours,crew_size,created_at").eq("customer_id", customerId).order("created_at", { ascending: false }),
...
function SignatureDialog({ estimate, defaultName, onClose, onSigned }: { estimate: Estimate; defaultName: string; onClose: () => void; onSigned: () => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/70 p-4"><div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-emerald-700">SIGNATURE ÉLECTRONIQUE</p><h2 className="mt-1 text-2xl font-bold">{estimate.estimate_number}</h2><p className="text-slate-600">{estimate.title || "Devis de service"} · {formatCad(estimate.total)}</p></div><button onClick={onClose} className="rounded-lg border px-3 py-2 text-sm">Fermer</button></div>{error && <Alert text={error} />}<div className="mt-6 space-y-5"><label className="block text-sm font-semibold">Nom complet du signataire<input value={name} onChange={(event) => setName(event.target.value)} maxLength={160} className="mt-2 w-full rounded-lg border px-3 py-2.5" /></label><div><div className="mb-2 flex items-center justify-between"><label className="text-sm font-semibold">Dessinez votre signature</label><button type="button" onClick={clear} className="inline-flex items-center text-sm font-semibold text-blue-700"><RotateCcw className="mr-1 h-4 w-4" />Effacer</button></div><canvas ref={canvasRef} onPointerDown={start} onPointerMove={move} onPointerUp={stop} onPointerCancel={stop} className="h-[180px] w-full touch-none rounded-lg border-2 border-dashed bg-slate-50" /></div><label className="flex items-start gap-3 rounded-lg border bg-slate-50 p-4 text-sm"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} className="mt-1 h-4 w-4" /><span>Je confirme avoir examiné et accepter ce devis, sa durée estimée, ses prix et ses conditions. Ma signature et la réservation d’un créneau constituent un engagement à faire exécuter le service et à payer la facture conformément aux conditions convenues. Tout travail supplémentaire hors du périmètre convenu nécessite mon approbation préalable.</span></label><p className="text-xs text-slate-500">La date, l’heure, le compte client, l’adresse IP disponible et le navigateur seront enregistrés comme preuve de l’acceptation.</p><button type="button" onClick={() => void sign()} disabled={saving || !consent || !hasSignature || !name.trim()} className="w-full rounded-lg bg-emerald-600 px-4 py-3 font-semibold text-white disabled:opacity-50">{saving ? "Enregistrement…" : "Signer et accepter le devis"}</button></div></div></div>
}

function BookingPanel({ estimate, onBooked }: { estimate: Estimate; onBooked: () => Promise<void> }) {
  const [day,setDay]=useState(()=>{const d=new Date();d.setDate(d.getDate()+1);return d.toISOString().slice(0,10)}); const [slots,setSlots]=useState<{slot_start:string;slot_end:string}[]>([]); const [busy,setBusy]=useState(false); const [msg,setMsg]=useState("");
  async function loadSlots(){setBusy(true);setMsg("");const {data,error}=await supabase.rpc("get_available_slots",{p_estimate_id:estimate.id,p_day:day});setBusy(false);if(error){setMsg(error.message);return}setSlots((data??[]) as {slot_start:string;slot_end:string}[])}
  async function book(start:string){setBusy(true);setMsg("");const {error}=await supabase.rpc("customer_book_estimate",{p_estimate_id:estimate.id,p_start:start});setBusy(false);if(error){setMsg(error.message);return}setMsg("Rendez-vous réservé. Ottawa Multiservices Group confirmera l’intervention.");setSlots([]);await onBooked()}
  return <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4"><p className="font-bold text-blue-950">Choisissez votre rendez-vous</p><p className="mt-1 text-sm text-blue-900">Les créneaux proposés tiennent compte de la durée estimée de {estimate.estimated_duration_hours ?? "—"} h.</p><div className="mt-3 flex gap-2"><input type="date" value={day} onChange={e=>setDay(e.target.value)} className="rounded-lg border px-3 py-2"/><button type="button" onClick={()=>void loadSlots()} className="rounded-lg bg-blue-700 px-4 py-2 font-semibold text-white">Voir les disponibilités</button></div>{msg&&<p className="mt-3 text-sm font-medium">{msg}</p>}<div className="mt-3 flex flex-wrap gap-2">{slots.map(s=><button disabled={busy} key={s.slot_start} onClick={()=>void book(s.slot_start)} className="rounded-lg border bg-white px-3 py-2 text-sm font-semibold hover:bg-blue-100">{new Date(s.slot_start).toLocaleTimeString("fr-CA",{hour:"2-digit",minute:"2-digit"})}</button>)}</div></div>
}
