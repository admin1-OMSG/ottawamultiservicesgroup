...
export function QuoteFunnel({ startWith }: { startWith?: "client" | "partner" }) {
  ...
  const setA = (k: string, v: string | string[]) => setAnswers((p) => ({ ...p, [k]: v }));
...
function ServiceQuestions({ serviceKey, answers, setA }: { serviceKey: ServiceKey; answers: Record<string, string | string[]>; setA: (k: string, v: string | string[]) => void; }) {
...
type QItem = { id: string; label: string; kind: "radio" | "select" | "multi"; options: string[] };

function QuestionField({ item, value, onChange }: { item: QItem; value: string | string[]; onChange: (v: string | string[]) => void }) {
  if (item.kind === "multi") {
    const selected = Array.isArray(value) ? value : value ? [value] : [];
    return <div><Label className="text-sm font-semibold text-navy">{item.label} <span className="font-normal text-muted-foreground">(choose all that apply)</span></Label><div className="mt-3 grid gap-2 sm:grid-cols-2">{item.options.map((o) => { const active=selected.includes(o); return <label key={o} className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 ${active ? "border-accent bg-accent/5" : "border-border"}`}><input type="checkbox" checked={active} onChange={() => onChange(active ? selected.filter(x=>x!==o) : [...selected,o])} /><span className="text-sm">{o}</span>{active && <CheckCircle2 className="ml-auto h-4 w-4 text-accent"/>}</label>})}</div></div>;
  }
  if (item.kind === "select") {
    return (
      <div>
  ...
  return (
    <div>
      <Label className="text-sm font-semibold text-navy">{item.label}</Label>
      <RadioGroup value={typeof value === "string" ? value : ""} onValueChange={onChange} className="mt-3 grid gap-2 sm:grid-cols-2">
        {item.options.map((o) => {
...
function questionsFor(key: ServiceKey, a: Record<string, string | string[]>): QItem[] {
  switch (key) {
    case "cleaning": {
      const base: QItem[] = [
    ...
    case "detailing":
      return [
        ...
        { id: "extras", label: "Do you need any extra services?", kind: "multi", options: ["None","Odor Removal","Pet Hair Removal","Headlight Restoration","Ceramic Coating"] },
      ];
    case "lawn":
      return [
        { id: "lawnService", label: "What service do you need?", kind: "multi", options: ["Lawn Mowing","Hedge Trimming","Spring Cleanup","Fall Cleanup","Weed Removal","Garden Maintenance"] },
    ...
    case "snow":
      return [
        { id: "snowArea", label: "What do you need cleared?", kind: "multi", options: ["Driveway","Walkway","Entire Property","Roof Raking"] },
    ...
    case "commercial-snow":
      return [
        ...
        { id: "coverage", label: "Coverage needed", kind: "multi", options: ["Parking lot","Walkways","Full property + salting"] },
      ];
    case "commercial-lawn":
      return [
        { id: "propertyType", label: "Property type", kind: "radio", options: ["Retail plaza","Office building","Industrial lot","Condo / multi-unit"] },
        { id: "scope", label: "Scope of service", kind: "multi", options: ["Mowing only","Full landscape maintenance","Seasonal cleanups"] },
      ];
    case "property":
      return [
